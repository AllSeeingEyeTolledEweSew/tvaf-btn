...
class TestPathStructure(BaseFTPTest):
    ...
    def test_LIST_with_symlinks(self):
        self.ftp.cwd(f"/v1/{ltu.SINGLE.infohash}/test/f")
        lines = []
        self.ftp.retrlines("LIST", callback=lines.append)
    ...
    def test_torrent_dir(self):
        ...
        self.ftp.cwd(f"/v1/{ltu.SINGLE.infohash}/test/i")
        self.assert_mlsd("", ("perm", "size", "type"), [
            ("0",
             dict(perm="r", type="file", size=str(ltu.SINGLE.files[0].length)))
        ])
    ...
    def test_CWD_invalid(self):
        with self.assertRaisesRegex(ftplib.error_perm, "550 Not a directory."):
            self.ftp.cwd(f"/v1/{ltu.SINGLE.infohash}/test/i/0")

    def test_SIZE(self):
        # pyftpdlib does not allow SIZE in ascii mode to avoid newline
        # translation issues
        self.ftp.voidcmd("TYPE I")
        size = self.ftp.size(f"/v1/{ltu.SINGLE.infohash}/test/i/0")
...
class TestReadOnly(BaseFTPTest):
    ...
    def test_STOR_overwrite(self):
        with self.assertRaisesRegex(ftplib.error_perm, "550 .*"):
            self.ftp.storbinary(f"STOR /v1/{ltu.SINGLE.infohash}/test/i/0",
                                io.BytesIO(b"data"))

    def test_APPE(self):
        with self.assertRaisesRegex(ftplib.error_perm, "550 .*"):
            self.ftp.storbinary(f"APPE /v1/{ltu.SINGLE.infohash}/test/i/0",
                                io.BytesIO(b"data"))

    def test_DELE(self):
        with self.assertRaisesRegex(ftplib.error_perm, "550 .*"):
            self.ftp.delete(f"/v1/{ltu.SINGLE.infohash}/test/i/0")

    def test_rename(self):
        with self.assertRaisesRegex(ftplib.error_perm, "550 .*"):
            self.ftp.rename(f"/v1/{ltu.SINGLE.infohash}/test/i/0",
                            f"/v1/{ltu.SINGLE.infohash}/test/i/123")
    ...
    def test_MFMT(self):
        with self.assertRaisesRegex(ftplib.error_perm, "550 .*"):
            self.ftp.voidcmd(
                f"MFMT 19700101032545 /v1/{ltu.SINGLE.infohash}/test/i/0")
...
class TestRETR(BaseFTPTest):
    ...
    def test_RETR(self):
        buf = io.BytesIO()
        self.ftp.retrbinary(f"RETR /v1/{ltu.SINGLE.infohash}/test/i/0",
                            buf.write)
    ...
    def test_RETR_dir(self):
        buf = io.BytesIO()
        with self.assertRaisesRegex(ftplib.error_perm, "550 .*"):
            self.ftp.retrbinary(f"RETR /v1/{ltu.SINGLE.infohash}/test/i",
                                buf.write)
