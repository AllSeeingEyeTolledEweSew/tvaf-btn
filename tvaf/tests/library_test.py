...
class TestLibraryService(unittest.TestCase):
    ...
    def setUp(self):
        self.torrents = {
            t.infohash: t
            for t in (SINGLE, MULTI, PADDED, CONFLICT_FILE, CONFLICT_FILE_DIR,
                      CONFLICT_DIR_FILE, BAD_PATHS)
        }
        self.hints = {
            (SINGLE.infohash, 0):
                library.Hints(mime_type="text/plain"),
            (MULTI.infohash, 0):
                library.Hints(mime_type="application/x-tar",
    ...
    def assert_dirents_like(self, dirents: Iterable[fs.Dirent],
        ...
        got = [(stat_lib.filemode(d.stat.filetype), d.stat.mtime, d.name)
               for d in dirents]
    ...
    def test_get_hints_func_fails(self):
        ...
        tfile = cast(library.TorrentFile,
                     self.libs.root.traverse(f"v1/{SINGLE.infohash}/test/i/0"))
    ...
    def test_hints_with_mtime(self):
        ...
        by_index = cast(fs.Dir,
                        self.libs.root.traverse(f"v1/{SINGLE.infohash}/test/i"))
    ...
    def test_browse(self):
        ...
        test_dir.mkchild(
            "single",
            fs.Symlink(target=self.libs.lookup_torrent(SINGLE.infohash)))
    ...
    def test_torrent_dir_with_redirect_access(self):
        ...
        torrent_dir = cast(fs.Dir,
                           self.libs.root.traverse(f"v1/{SINGLE.infohash}"))
        self.assert_dirents_like(torrent_dir.readdir(),
                                 [("l", None, "redirect"), ("d", None, "test")])
    ...
    def test_access_readdir(self):
        for info_hash in self.torrents:
            ...
            self.assert_dirents_like(access.readdir(), [("d", None, "f"),
                                                        ("d", None, "i")])
    ...
    def test_by_path_single(self):
        by_path = cast(fs.Dir,
                       self.libs.root.traverse(f"v1/{SINGLE.infohash}/test/f"))
    ...
    def test_by_path_multi(self):
        by_path = cast(fs.Dir,
                       self.libs.root.traverse(f"v1/{MULTI.infohash}/test/f"))
    ...
    def test_by_index_multi(self):
        by_index = cast(fs.Dir,
                        self.libs.root.traverse(f"v1/{MULTI.infohash}/test/i"))
    ...
    def test_conflict_file(self):
        ...
        by_index = cast(
            fs.Dir,
            self.libs.root.traverse(f"v1/{CONFLICT_FILE.infohash}/test/i"))
        ...
        for i in range(len(CONFLICT_FILE.files)):
            ...
            self.assert_torrent_file(tfile, dummy=CONFLICT_FILE, dummy_file=i)
    ...
    def test_conflict_file_dir(self):
        ...
        by_index = cast(
            fs.Dir,
            self.libs.root.traverse(f"v1/{CONFLICT_FILE_DIR.infohash}/test/i"))
    ...
    def test_conflict_dir_file(self):
        ...
        by_index = cast(
            fs.Dir,
            self.libs.root.traverse(f"v1/{CONFLICT_DIR_FILE.infohash}/test/i"))
    ...
    def test_bad_paths(self):
        ...
        # empty.
        by_path = cast(
            fs.Dir, self.libs.root.traverse(f"v1/{BAD_PATHS.infohash}/test/f"))
        self.assert_dirents_like(by_path.readdir(), [])

        by_index = cast(
            fs.Dir, self.libs.root.traverse(f"v1/{BAD_PATHS.infohash}/test/i"))
        ...
        for i in range(len(BAD_PATHS.files)):
            tfile = cast(library.TorrentFile, by_index.lookup(str(i)))
            self.assert_torrent_file(tfile, dummy=BAD_PATHS, dummy_file=i)

    def test_padded(self):
        by_path = cast(
            fs.Dir,
            self.libs.root.traverse(f"v1/{PADDED.infohash}/test/f/padded"))
        self.assert_dirents_like(by_path.readdir(), [("l", None, "file.tar.gz"),
                                                     ("l", None, "info.nfo")])

        by_index = cast(fs.Dir,
                        self.libs.root.traverse(f"v1/{PADDED.infohash}/test/i"))
        ...
        for i in range(len(PADDED.files)):
            if b"p" in PADDED.files[i].attr:
                ...
                tfile = cast(library.TorrentFile, by_index.lookup(str(i)))
                self.assert_torrent_file(tfile, dummy=PADDED, dummy_file=i)
